...
| MPU spike | ServiceTask/ImuService | SystemTask/telemetry | `0x68`，WHO `0x68/0x70` Profile 1，100 Hz | 与 OLED 共享 I2C；offline 立即 invalid，重新探测/校准后 READY；OLED+UART 邻居 27000 秒静态回归通过；正式 ICM42688 需重新验收 |
...
5. Phase 2A 与 MPU spike 当前位于不同工作树，尚未执行正式代码组合；不能因各自单模块通过就
   宣称两者已经可一起运行。
6. MPU spike 当前为 `bench_passed / pairwise_passed`：OLED + MPU + UART 邻居组合已完成
   27000 秒静态回归，CRC/sequence/deadline/I2C/Health 全 0；但尚未完成
   Motion、物理恢复、示波器上升沿或与 Phase 2A 的正式组合，因此仍禁止写 `integration_ready`。
...
- 当前 `pairwise_passed` 只覆盖 OLED + MPU6xxx + UART telemetry 的共享 I2C/quiet window 组合；
  27000 秒全零门禁不能替代与 Phase 2A、电机噪声或正式线束的组合测试。
- 后续增加传感器时复用总线 owner 和有界事务，不为每个设备新增 task/mutex/ISR；每个事务继续保持
  长度上限、超时、明确 STOP 和失败后的降级重试。
- Health 帧已经达到 128 B 上限。原始六轴、温度和姿态估计应使用独立版本化帧，禁止继续向
  Health payload 追加字段，也禁止复用当前 Control 四浮点语义作为正式长期协议。
- 新算法层按 `device raw -> calibration -> bounded filtering -> quaternion estimator -> snapshot`
  分离；device 驱动不保存姿态，任务不直接实现滤波或 KF/EKF。
- 每次算法板测都重跑 100/1 Hz、CRC/gap/drop、deadline、I2C、OLED、quiet、stack、heap 和
  ring 门禁。没有外部航向参考时，长期 yaw 只能标为 unobservable，不能写成零漂已消除。
